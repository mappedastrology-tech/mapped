...
export default function TermsPage() {
  return (
    <LegalShell title="Terms of Service" updated="June 19, 2026">
      ...
      {/*
        Where usage limits belong.
        The plan copy used to say "Unlimited Dolly — ask as much as you like,
        subject to fair use", which was false; a stint naming the exact daily
        figure on the pricing card replaced one problem with another, since no
        subscription quotes a message count and doing so reads as a ration.
        The figures live here, linked from the paywall and from account
        settings, and the app says plainly when someone actually reaches one.
      */}
      <LegalH2>6. Limits on AI features</LegalH2>
      <p>
        Dolly, the ritual wizard, journal reflections, palm readings and horoscopes are generated by a
        third-party AI provider, and each request costs us money. So that the Service stays available and
        affordable for everyone, paid plans include generous but finite usage rather than unlimited usage.
      </p>
      <p>
        There is a per-day cap on how many messages you can send Dolly and how many rituals the wizard will
        generate, and a monthly ceiling on total AI use across the Service. The great majority of subscribers
        never come near either. These figures are set to keep the Service sustainable and we may adjust them;
        we will not reduce them in a way that materially changes what you are paying for without telling you
        first. If you do reach a limit, the app will say so clearly and tell you when it resets, and everything
        in Mapped that isn&rsquo;t AI-generated keeps working.
      </p>
      <p>
        These limits exist to prevent abuse and runaway cost, not to meter ordinary use. Automated, scripted or
        bulk access to the AI features is not permitted &mdash; see Acceptable use below.
      </p>
