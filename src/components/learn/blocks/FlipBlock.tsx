 ...
 */
export default function FlipBlock({ prompt, instructions, cards, back }: {
  prompt: string;
  instructions?: string;
  cards: Card[];
  /**
   * Artwork for the face-down side. Optional: no lesson authors a flip block
   * yet, and the ornate fallback below suits a non-tarot set better than a
   * tarot back would. Tarot lessons can pass /images/tarot/backside.webp.
   */
  back?: string;
}) {
  ...
  return (
    <div style={{ borderRadius: 18, padding: 16, background: "var(--lib-card)", border: "1px solid var(--lib-card-border)", boxShadow: "var(--lib-card-shadow)" }}>
      ...
      <p className="text-[12px] text-center mt-2" style={{ color: turned === cards.length ? "var(--sage-bright)" : "var(--lib-muted)", transition: "color .2s ease" }}>
      ...
      <div className="grid grid-cols-3 gap-2.5 mt-4">
        {cards.map((c, i) => {
          ...
          return (
            <button key={i} onClick={() => flip(i)} aria-label={isFlipped ? c.name : `Card ${i + 1}, tap to reveal`} style={{ perspective: 820, height: 182 }} className="active:scale-[0.98] transition-transform">
              <div style={{ position: "relative", width: "100%", height: "100%", transformStyle: "preserve-3d", transition: "transform 0.6s cubic-bezier(.2,.7,.2,1)", transform: isFlipped ? "rotateY(180deg)" : "none" }}>
                ...
                <div style={{ position: "absolute", inset: 0, backfaceVisibility: "hidden", WebkitBackfaceVisibility: "hidden", borderRadius: 11, overflow: "hidden", background: "radial-gradient(circle at 50% 32%, #3a2550, #1f1730 70%)", boxShadow: "0 10px 22px -10px rgba(0,0,0,0.7), inset 0 0 0 0.5px rgba(201,169,97,0.5)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  {back ? (
                    ...
                    <img src={back} alt="" aria-hidden="true" loading="lazy" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                  ) : (
      ...
      {/* caption */}
      <div className="flex items-center gap-3 mt-4 px-4 py-3" style={{ borderRadius: 16, background: "var(--lib-plum-island)", boxShadow: "inset 0 0 0 0.5px rgba(201,169,97,0.4)", minHeight: 30 }}>
        {cap ? (
          <>
            ...
            <div className="min-w-0">
              <div className="text-[15px] leading-tight" style={{ fontFamily: "var(--font-serif-lib)", color: "var(--lib-on-plum)" }}>{cap.name}</div>
              {cap.caption && <div className="text-[11.5px] mt-0.5" style={{ color: "rgba(240,230,210,0.72)" }}>{cap.caption}</div>}
            </div>
          </>
        ) : (
          <>
            <span className="shrink-0 text-center" style={{ color: "var(--brass-light)", fontSize: 18, minWidth: 22 }} aria-hidden="true">✦</span>
            <div className="min-w-0">
              <div className="text-[13px] leading-tight" style={{ color: "var(--lib-on-plum)" }}>Turn a card to read its meaning</div>
              <div className="text-[11.5px] mt-0.5" style={{ color: "rgba(240,230,210,0.72)" }}>Each card names a chapter of the journey.</div>
            </div>
          </>
        )}
      </div>
